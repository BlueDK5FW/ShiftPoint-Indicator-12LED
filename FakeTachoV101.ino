...
unsigned long loopCnt,beforeTime,elapsedTime,acceleroTime;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
void setup(){
  beforeTime = elapsedTime = acceleroTime = 0;
  ...
  if (carSpeed >= 5) {  //5km以上
    for  (i = 5; i < 14; i++){  // LED4～
      if (rpm > IDLE_RPM) {  //800回転は無視  
        if (rpm >= IDLE_RPM + ((i-4) * (STEP_RPM * 1.1))) {  //点灯補正：USB接続時：0.6、USB非接続時：1.1
          digitalWrite(i,HIGH);
        } else {    //STEPに満たない時
